...
public class Device
{
    ...
    private static boolean doesDeviceSupportExtensions(PhysicalDeviceBlueprint blueprint) //check with the last calls extensions
    {
        try(MemoryStack stack = stackPush())
        {
            ...
            VkExtensionProperties.Buffer allExtensions = VkExtensionProperties.malloc(count.get(0), stack);
    ...
    public Device(PhysicalDeviceBlueprint dev, long surface)
    {
        ...
        try(MemoryStack stack = stackPush())
        {
            ...
            VkDeviceQueueCreateInfo.Buffer queueCreateInfos = VkDeviceQueueCreateInfo.calloc(queueFamilies.length, stack);
            ...
            deviceFeatures.geometryShader(true); //TODO write finction to do that
    ...
    public void copyBuffer(long srcBuffer, long dstBuffer, int offset, int size)
    {
        try(MemoryStack stack = stackPush())
        {
            VkCommandBuffer commandBuffer = beginSingleTimeCommands(stack);
            ...
            VkBufferCopy.Buffer copyRegion = VkBufferCopy.calloc(1, stack);
            copyRegion.srcOffset ( offset );  // Optional
    ...
    public void freeBuffer(long bufferP, long memP)
    {
        vkDestroyBuffer(getLogical(), bufferP, null);
        vkFreeMemory   (getLogical(), memP, null);
    }
