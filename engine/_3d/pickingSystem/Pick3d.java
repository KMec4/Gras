...
public class Pick3d
{
    ...
    int pickingSystemEndRendering(VkCommandBuffer commandBuffer)
    {
        ...
        int id = loadImage(dev, window.getWindow());
        if(id < 0)
        {
            return -1;
        }
    ...
    VkCommandBuffer pickingSystemStartRendering(Matrix4f camMatrix)
    {
        if(!imgExtent2d.equals(window.getSize()))
        {
            System.out.println(" -> recreate Framebuffer");
            recreateFramebuffer(dev, window.getSize());
        }
        VkCommandBuffer cmd = execCommandBuffer(view, dev, window.getSize());
    ...
    void createRenderPass(Device dev)
    {
        try(MemoryStack stack = stackPush())
        {
            ...
            VkAttachmentDescription.Buffer colorAttachmentB = VkAttachmentDescription.calloc(1, stack);
            ...
            VkAttachmentReference.Buffer colorAttachmentRefB = VkAttachmentReference.calloc(1, stack);
            ...
            VkSubpassDescription.Buffer subpassB = VkSubpassDescription.calloc(1, stack);
    ...
    void createFramebuffer(Device dev, VkExtent2D dim)
    {
        try(MemoryStack stack = stackPush())
        {
            ...
            VkImageViewCreateInfo viewInfoD = VkImageViewCreateInfo.calloc(stack);
    ...
    VkCommandBuffer execCommandBuffer(Viewport view, Device dev, VkExtent2D d)
    {
        try(MemoryStack stack = stackPush())
        {
            ...
            renderPassInfo.renderArea(VkRect2D.calloc(stack).offset(offset).extent(d));
